...
int main()
{
    ...
    {
        Rect rect;
        /* Поля класса проинициализировались мусорными значениями как локальные переменные.
        
        При объявлении объекта вызывается конструктор - особая функция (не возвращающая никакого значения), 
        которая в соответствии с переданными параметрами (если такие есть) инициализирует поля класса. 
        В данном случае, поскольку конструктор класса Rect явно не определен, то он создается компилятором 
        автоматически и вызывает конструкторы полей класса.
        
        В общем случае конструктор также вызывает конструкторы по умолчанию базовых классов (при наличии
        таких классов и таких конструкторов у них). */
    }

    /**
     * Задание 1.3. Явно определенные конструкторы. Конструктор по умолчанию.
     *
     * Объявите в файле `rect.hpp` конструктор для класса `Rect`, который
    ...
    {
        Rect rect(1, 2, 3, 4);
        /* Ошибка компиляции возникает в том случае, когда конструктор класса объявлен в заголовочном файле
        и больше нигде не определен. */
    }
    ...
    {
        Rect rect1(1, 2, 3, 4); // здесь вызывается явно определенный конструктор
        Rect rect2 = rect1;     // здесь неявно вызывается конструктор копирования
        Rect rect3(rect1);      // и здесь конструктор копирования, но вызывается он явно
        /* В нашем случае конструктор копирования определен автоматически. Он выполняет поэлементное
        копирования полей класса в новый объект. */
    }
    ...
    {
        /* Деструктор - функция, в некотором смысле "обратная" конструктору, так как она отвечает за 
        освобождение динамической памяти, занимаемой полями класса. Деструктор автоматически вызывается
        при выходе из области видимости объекта.
        Поскольку в нашем примере деструктор изначально не определен, то компилятор создает его пустым. 
        
        В предыдущих заданиях деструктор вызвался для всех пяти объектов при выходе из каждого блока кода,
        в котором эти объекты определены. */
    }
...

    {
        ...
        Rect *pR = new Rect(1,2,1,2); // конструктор
        {
            ...
            Rect arRect[2]; // конструктор по умолчанию (2 раза)
            for(int i = 0; i < 3; i++)
            {
                static Rect r3(i, i, i, i); // конструктор
                Rect r4(*pR); // конструктор копирования
                Rect r5(i, i, i, i); // конструктор
            } // деструкторы для r4 и r5
        } // деструкторы для r2, arRect[0] и arRect[1] 
        delete pR; // деструктор для pR
    } // деструктор для r1
    // деструктор для r3 вызывается при завершении программы
...

    {
        ...
        /* Приватные поля и функции доступны для использования только внутри самого класса */
    }
     ...
     */
    {
        /* Основные отличия функций-членов от обычных функций:
        1. Функции-члены принадлежат конкретному классу, т. е. по умолчанию имеют доступ ко 
        всем полям и функциям этого класса (открытым и закрытым), а также принимают в качестве 
        неявного аргумента указатель `this`.
        2. Область видимости функции-члена ограничивается областью видимости класса. */
    }
    ...
    {
        Rect r1(1, 2, 3, 4), r2(5, 6, 7, 8); // обычные пользовательские конструкторы
...

    {
        ...
        cout << "r1: "; print_rect(r1);
        cout << "Ширина: " << r1.get_width() << ". Высота: " << r1.get_height() << ". Площадь: " << r1.get_square() << endl;
        cout << "Введите новые значения ширины и высоты: ";
        int w, h;
        cin >> w >> h;
        r1.set_width(w); r1.set_height(h);
        cout << "Ширина: " << r1.get_width() << ". Высота: " << r1.get_height() << ". Площадь: " << r1.get_square() << endl;

        cout << "r2: "; print_rect(r2);
        cout << "Ширина: " << r2.get_width() << ". Высота: " << r2.get_height() << ". Площадь: " << r2.get_square() << endl;
        cout << "Введите новые значения ширины и высоты: ";
        cin >> w >> h;
        r2.set_width(w); r2.set_height(h);
        cout << "Ширина: " << r2.get_width() << ". Высота: " << r2.get_height() << ". Площадь: " << r2.get_square() << endl;
    ...
    {
        Barrel alch_b(100*1000, 96); // бочка со спиртом
        Barrel water_b(100*1000);

        int i = 0;
        while(alch_b.get_percent() >= 50)
        {
            alch_b.pour_from(water_b, 1000);
            water_b.pour_from(alch_b, 1000);
            i++;
        }
        cout << "Понадобилось " << i << " итераций\n";
    }
    ...
    {
        /* Константная матрица */
        Matrix m_const(2);    //    | 1 1 |
        m_const.set(1, 1, 0); //    | 1 0 |
        /* Матрица-вектор */
        Matrix vector(2, 1); //     | 1 |  или  | F[n-1] |
        vector.set(0, 0, 1); //     | 0 |       | F[n-2] |
        /* Матрица-результат */
        Matrix result(2, 1); //     |  F[n]  |  
                             //     | F[n-1] |  (изначально нулевая)
        
        const int fib_numbers = 40; // количество выводимых чисел Фибоначчи
        const int limit = 5; // максимальное допустимое количество чисел в одной строке (для удобства отображения)
        cout << "Первые " << fib_numbers << " чисел Фибоначчи.\n";
        for (int i = 1; i <= fib_numbers; i++)
        {
            if (i % limit == 1) printf("(%d - %d): ", i, i+4);
            if (i <= 2) cout << vector.get(2-i, 0) << ' ';
            else
            {
                result = m_const.multiply(vector);
                cout << result.get(0, 0) << ' ';
                if (i % limit == 0) cout << endl;

                /* Переход на следующую итерацию */
                vector.set(0, 0, result.get(0, 0));
                vector.set(1, 0, result.get(1, 0));
            }
        }
    }
     ...
     * используют их для реализации логики более высокого уровня.
     *
    ...
    {
        MyString s1("Ob'ect - obladatel' resursa.");
        cout << "s1: "; s1.print(); cout << endl;
        cout << "10 символ строки - '" << s1.get(9) << "'\n";
    }
     ...
     * отладочной версии вашей программы вам нужно выполнить программу
     ...
     * и `build/text_wrapper.o`. После этого создайте правило для
     * отладочной сборки:
     *
     * ```
    ...
    {
        /* 
        1. Добавил переменные, хранящие перечисления имен объектных файлов, засчет чего
        сильно сократилось количество кода и увеличилась его читаемость.
        2. Добавил цель `clean`, выполняющую очистку папки `build`.
        */
    }
     ...
     * некоторых целей не очень удобно задавать прямоугольник левой, правой, нижней и верхней
